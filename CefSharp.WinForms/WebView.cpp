...
namespace WinForms
{
	...
	void WebView::OnHandleCreated(EventArgs^ e)
	{
		if (DesignMode == false) 
		{
			if (windowInjection->Handle != IntPtr.Zero)
			{
				windowInjection->ReleaseHandle();
			}
	...
	void WebView::OnInitialized()
	{
		...
		windowInjection->AssignHandle(IntPtr(_clientAdapter->GetBrowserHwnd()));
	...
	void WebView::OnFrameLoadEnd(String^ url)
	{
		...
		LoadCompleted(this, gcnew LoadCompletedEventArgs(url));		
	}
